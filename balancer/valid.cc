...
bool Balancer::valid()
{
	...
	for (size_t idx1 = 0; idx1 < d_lanes.size() - 1; ++idx1)
		for (size_t idx2 = idx1 + 1; idx2 < d_lanes.size(); ++idx2)
			if (sharedSplitters(d_lanes[idx1], d_lanes[idx2]) > 1)
				return false;
	
	vector<char> secondVector(d_cols * d_rows, 0);
	for (vector<Lane>::iterator it1 = d_lanes.begin(); it1 != d_lanes.end(); ++it1)
