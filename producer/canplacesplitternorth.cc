...
bool Producer::canPlaceSplitterNorth(vector<char> &matrix, size_t idx, size_t cols, size_t n)
{
	...
	if (idx == 2 * cols and matrix[idx - cols + 1] == SPLN and matrix[idx - cols] == EMPTY)
		return false;

	if (idx == matrix.size() - 2 * cols + 1 and matrix[idx - cols] == SPRN and matrix[idx - 1] == EMPTY)
		return false;

	if (idx == 3 * cols - 2 and matrix[idx - cols] == SPRN and matrix[idx - cols + 1] == EMPTY)
		return false;
